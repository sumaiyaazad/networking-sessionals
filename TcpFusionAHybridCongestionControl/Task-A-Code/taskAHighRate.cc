...
{
    //NS_LOG_UNCOND (i<<" node: "<<Simulator::Now ().GetSeconds () << "\t" << newCwnd);
...
int main(int argc, char *argv[])
{
    ...
    uint32_t rate = (payloadSize * 500 * 8) / 1000000; 
    std::ostringstream rateString;
    rateString << rate << "Mbps";
    std::string dataRate = rateString.str();       /* Application layer datarate. */
    std::string tcpVariant = "TcpFusion"; /* TCP variant type. */
    std::string phyRate = "HtMcs7";        /* Physical layer bitrate. */
    double simulationTime = 15;            /* Simulation time in seconds. */
    ...
    cmd.AddValue("left", "Number of \"extra\" CSMA nodes/devices", leftnodes);
    cmd.AddValue("right", "Number of wifi STA devices", rightnodes);
    cmd.AddValue("num_half_flows", "how may flow", num_half_flows);
    ...
    // YansWifiChannelHelper channelLeft = YansWfiChannelHelper::Default();
    // channelLeft.SetPropagationDelay("ns3::RangePropagationLossModel::MaxRange", DoubleValue(10.0));
    Config::SetDefault ("ns3::RangePropagationLossModel::MaxRange", DoubleValue (7.0*6));
    ...
    /* Set up Legacy Channel */
    // YansWifiChannelHelper channelRight=YansWfiChannelHelper::Default();
    // channelRight.SetPropagationDelay("ns3::RangePropagationLossModel::MaxRange", DoubleValue("10.0"));
    YansWifiChannelHelper channelRight;
    //channelRight.AddPropagationLoss("ns3::RangePropagationLossModel","MaxRange", DoubleValue(10.0));
    ...
    for (uint32_t i = 1; i <= num_half_flows; i++)
    {
        ...
        oss << "./HighRate/flow" << i  << ".cwnd";
        
        Ptr<OutputStreamWrapper> stream = asciiTraceHelper.CreateFileStream(oss.str());
        ns3TcpSocket->TraceConnectWithoutContext("CongestionWindow", MakeBoundCallback(&CwndChange, stream));
        std::ostringstream os;
        os << "./simulation_output/throughput" << i  << ".throughput";
        stream = asciiTraceHelper.CreateFileStream(os.str());
        Simulator::Schedule(Seconds(1.1), &CalculateThroughput, sink, stream, 0);
    }
    ...
    */
    ...
    Simulator::Destroy();
    /*
NS_LOG_UNCOND("Total packets send : "<<total_packets_sent);
NS_LOG_UNCOND("Total packets dropped : "<<total_packets_dropped);
NS_LOG_UNCOND("Total packets received : "<<total_packets_received);

NS_LOG_UNCOND("packet delivery ratio : "<<total_packets_received/(float)total_packets_sent);
NS_LOG_UNCOND("packet drop ratio : "<<total_packets_dropped/(float)total_packets_sent);*/
